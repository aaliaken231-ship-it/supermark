...
// Grace period for expired licenses before complete lockout (in milliseconds)
 ...
 * Each license type includes: label, duration, price, description, color scheme, and features
 */
...
export default function Licenses() {
  ...
  const loadLicenses = useCallback(async () => {
    try {
      ...
      if (systemData.length > 0) {
        ...
        await put(STORES.SYSTEM, {
          ...systemData[0],
          lastKnownTime: new Date().toISOString()
        });
      } else {
        await add(STORES.SYSTEM, {
          lastKnownTime: new Date().toISOString(),
   ...
   * Redirects unauthorized users or checks authentication
   */
  const checkSession = () => {
    try {
      ...
      // Check authorization - only admins can access licenses
      ...
      setSession(user);
      setIsSuperAdmin(user.user_type === 'SuperAdmin');
      loadLicenses();
    } catch (error) {
  ...
  const handleCreateLicense = async () => {
    ...
    const licenseData = {
      ...
      role: session?.user_type || 'Administrateur',
    ...
    const safeShopName = shopName.replace(/[^a-z0-9]/gi, '_').toLowerCase();
    const fileName = `demande_licence_${safeShopName}_${selectedLicenseType}_${code.substring(0, 5)}.lic`;
    
    try {
      downloadFile(encryptedContent, fileName);
    } catch (error) {
  ...
  const handleGenerateLicense = async () => {
    ...
    const file = await selectFile('.lic');
    if (!file) return;
    ...
    reader.onload = async (event) => {
      try {
        ...
        const fileName = `licence_${days}j_${licenseType}_${requestData.code.substring(0, 5)}.lic`;
        downloadFile(encryptedLicense, fileName);
  ...
  // 3. Activate License (Now with Server-side validation)
    ...
    reader.onload = async (event) => {
      ...
      try {
        ...
        const response = await fetch('/api/license/activate', {
          ...
          body: JSON.stringify({ encryptedLicense: encrypted, machineId })
        });

        const result = await response.json();
        
        if (!response.ok) {
          toast.error("Erreur d'activation", { description: result.error || "Fichier invalide." });
          return;
        }

        const { expiresAt, activatedAt, licenseData } = result;
        const licenseType = licenseData.licenseType as keyof typeof LICENSE_TYPES || 'mensuelle';
        const typeInfo = LICENSE_TYPES[licenseType] || LICENSE_TYPES.mensuelle;

        // Check if license was already activated in local DB
        const existingLicenses = await db.getLicenses();
        const existingLicense = existingLicenses.find(l => l.code === licenseData.code);

        if (existingLicense && existingLicense.status === 'activated') {
          toast("Licence déjà active", { description: "Cette licence est déjà enregistrée localement." });
          return;
        }
        ...
        if (existingLicense) {
          ...
          await db.addLicense({
            ...
            duree_jours: licenseData.duration,
            prix: licenseData.price || typeInfo.price,
            machine_id: machineId,
            created_at: licenseData.generatedAt || activatedAt,
            activated_at: activatedAt,
            expires_at: expiresAt
          });
        }
        ...
        await db.addActivityLog({
          ...
          utilisateur: session.nom_utilisateur,
          date_action: activatedAt,
          details: `Licence ${typeInfo.label} activée via serveur: ${licenseData.code}`,
          module: 'Licences',
        ...
        toast.success("Licence activée avec succès!", {
          description: `${typeInfo.label} valide jusqu'au: ${new Date(expiresAt).toLocaleDateString('fr-FR')}`,
        });
      } catch (err) {
        console.error("Activation error:", err);
        toast.error("Erreur", { description: "Impossible de contacter le serveur d'activation." });
      }
    };
    reader.readAsText(file);
  };

  // 5. Streamlined Renewal
  const handleRenewLicense = async (license: License) => {
    const now = new Date().toISOString();
    const machineId = getMachineId();
    const licenseType = license.type_licence as keyof typeof LICENSE_TYPES;
    const typeInfo = LICENSE_TYPES[licenseType];

    const renewalData = {
      type: 'request',
      requestType: 'renewal',
      originalCode: license.code,
      code: generateLicenseCode(), // New code for the renewal
      email: PAYMENT_INFO.email,
      role: session?.user_type || 'Administrateur',
      licenseType: license.type_licence,
      days: typeInfo.days,
    ...
    const encryptedContent = encryptData(JSON.stringify(renewalData));
    
    // Save to local DB as pending
    await db.addLicense({
      code: renewalData.code,
      email: PAYMENT_INFO.email,
      role: session?.user_type || 'Administrateur',
      status: 'pending',
      type_licence: licenseType,
      duree_jours: typeInfo.days,
      prix: typeInfo.price,
      machine_id: machineId,
      created_at: now
    });
    ...
    const fileName = `renouvellement_${licenseType}_${license.code.substring(0, 5)}.lic`;
    downloadFile(encryptedContent, fileName);

    toast.success("Demande de renouvellement créée", {
      description: "Envoyez ce fichier à l'administrateur pour validation.",
    });
  ...
  const exportLicenses = () => {
    ...
    const fileName = `supermarket_licenses_backup_${new Date().toISOString().split('T')[0]}.json`;
                        ...
                        <TableRow key={index}>
                          ...
                          <TableCell>
                            <div className="flex items-center gap-1">
                              <Button variant="ghost" size="icon" onClick={() => copyToClipboard(license.code)} title="Copier le code">
                              ...
                              {license.status === 'activated' && (
                                <Button 
                                  ...
                                  title="Renouveler cette licence"
                                >
